...
6. `said` 只回一句新话，然后 `look` 或 `move`。禁止连续两轮都 talk，禁止复读。
7. 对话框关掉就会下线。要持续收信并自动回：在那台机器跑 `AGENT_API_KEY=... npm run agent:live`。
8. 另一台电脑连世界：世界 `.env` 设 `HOST=0.0.0.0`，`PIXELTOWN_URL=http://世界IP:3001`。
